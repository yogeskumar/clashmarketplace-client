  ...
  return (
    <div id="user">
      <div className="profile">
        <h1>name:&#9;<strong> {user_data.name}</strong></h1>
        <h1>email:&#9;<strong> {user_data.email}</strong></h1>
        <h1>password:&#9;<strong>{user_data.password
            ? user_data.password
            : "Null [Google account login]"}</strong>
        </h1>
      </div>
      {user_data.type === "admin" ? <button onClick={()=> history("/admin")}>Manage Accounts</button> : null}
      ...
      {user_data.owned_accounts.map((el, i) => (
        <div className="item">
          ...
          <h1>decription: {el.description}</h1>
